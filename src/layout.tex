...
% hyperxmp writes an XMP metadata stream (Dublin Core) mirroring the \hypersetup
% Info fields, so web crawlers and document systems can read title/author/
% subject/keywords. Vendored under ./vendor (see Makefile TEXINPUTS).
\usepackage{hyperxmp}
\usepackage{tikz}
...
\newcommand{\cvpaperwidth}{595pt}
\newcommand{\cvpaperheight}{842pt}
...
% PDF document metadata. LinkedIn and many HR/ATS tools read these fields;
% pdfkeywords carries role-relevant terms for keyword matching.
\hypersetup{
  pdftitle    = {Giacomo Marciani - Resume},
  pdfauthor   = {Giacomo Marciani},
  pdfsubject  = {Senior Software Engineer - High Performance Computing, Distributed Systems, Cloud Infrastructure},
  pdfkeywords = {Senior Software Engineer, High Performance Computing, HPC, AWS, Amazon Web Services, AWS ParallelCluster, NICE EnginFrame, Slurm, MPI, CUDA, Nvidia GPU, Lustre, EFA, Distributed Systems, Cloud Infrastructure, Infrastructure as Code, Terraform, CloudFormation, Kubernetes, CI/CD, Observability, Security, Threat Modeling, Big Data, Flink, Kafka, Hadoop, Python, Java, Technical Leadership, System Design, Mentoring},
  pdflang     = {en},
  % dc:source in the XMP stream; set to the canonical URL instead of the .tex filename.
  pdfsource   = {https://gmarciani.github.io/resume/resume.pdf}
}

% Prevent line-break hyphenation from splitting words in the text layer
% (e.g. "High Per-formance Computing"), which breaks ATS/AI keyword matching.
\hyphenpenalty=10000
...
\newcommand{\makecvheader}{%
  ...
  \tikz[remember picture,overlay]{%
    \node[anchor=north west, inner sep=0pt] at ([xshift=320pt, yshift=-18pt]current page.north west)
      {\includegraphics[height=50pt]{qrcode.png}};}%
...
% Show full URLs as visible text so text-only parsers capture them (not just
% the embedded hyperlink target).
\newcommand{\cvcontactlines}{%
  \iffield{\cvemail}{\href{mailto:\cvemail}{\cvemail}\\[\contactlinegap]}%
  \iffield{\cvlinkedin}{\href{https://linkedin.com/in/\cvlinkedin}{linkedin.com/in/\cvlinkedin}\\[\contactlinegap]}%
  \iffield{\cvgithub}{\href{https://github.com/\cvgithub}{github.com/\cvgithub}}%
}
...
% Page break that repeats the header on the new page
\newcommand{\cvnewpage}{\newpage\makecvheader}

% Section header: teal icon (from assets/images/) + teal uppercase label
% #1 = icon file (in assets/images/, without extension), #2 = label
\newcommand{\cvsection}[2]{%
  \resetbodygap
  \vspace{\sectiongap}%
  \noindent\raisebox{\cvsectioniconraise}{\includesvg[height=\cvsectioniconsize]{#1}}\hspace{\cvsectioniconsep}%
...
\long\def\body#1{%
  ...
  \noindent\begingroup\RaggedRight\setlength{\parindent}{0pt}\let\\\bodynewline #1\par\endgroup}
% Education row: bold degree, school + years on one line, grade on third line.
...
\newcommand{\makecv}{%
  ...
  % --- Page 1 ---
  \bodystart
    \cvsection{introduction}{SUMMARY}\cvintro
    \cvsection{work}{WORK EXPERIENCE}\cvworkleft
  \rightcolumn
    \cvworkright
    \cvsection{projects}{PROJECTS}\cvprojects\cvsectionfootnote
  ...
  \bodystart
    ...
    \cvsection{skills}{SKILLS}\cvskills
    \cvsection{honours}{AWARDS}\cvhonours
